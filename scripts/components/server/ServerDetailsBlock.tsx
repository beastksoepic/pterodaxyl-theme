...
const ServerDetailsBlock = () => {
    ...
    const diskLimit = limits.disk ? megabytesToHuman(limits.disk) : 'Unmetered';
    const memoryLimit = limits.memory ? megabytesToHuman(limits.memory) : 'Unmetered';
    const cpuLimit = limits.cpu ? limits.cpu + '%' : 'Unmetered';
    ...
    return (
        <TitledGreyBox css={tw`break-words`} title={name} icon={faServer}>
            ...
            <p css={tw`text-xs mt-2`}>
                ...
                <span css={tw`text-neutral-500`} style={{ color: 'gray' }}>
                    { cpuLimit !== 'Unmetered' ? ` / ${cpuLimit}` : ' - Unmetered'}
                </span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faMemory} fixedWidth css={tw`mr-1`}/> {bytesToHuman(stats.memory)}
                <span css={tw`text-neutral-500`} style={{ color: 'gray' }}>
                    { memoryLimit !== 'Unmetered' ? ` / ${memoryLimit}` : ' - Unmetered'}
                </span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faHdd} fixedWidth css={tw`mr-1`}/>&nbsp;{bytesToHuman(stats.disk)}
                <span css={tw`text-neutral-500`} style={{ color: 'gray' }}>
                    { diskLimit !== 'Unmetered' ? ` / ${diskLimit}` : ' - Unmetered'}
                </span>
            </p>
