                ...
                <Can action={'file.create'}>
                    <ErrorBoundary>
                        <div css={tw`flex flex-shrink-0 flex-wrap-reverse md:flex-nowrap justify-end mb-4 md:mb-0 ml-0 md:ml-auto`}>
                            <NewDirectoryButton css={tw`w-full flex-none mt-4 sm:mt-0 sm:w-auto sm:mr-2`}/>
                            <UploadButton css={tw`flex-1 mr-2 sm:flex-none sm:mt-0`}/>
                            <NavLink
                                to={`/server/${id}/files/new${window.location.hash}`}
                                css={tw`flex-1 sm:flex-none sm:mt-0`}
                            >
                                <Button css={tw`w-full`} size={'xsmall'} color={'primary'} isSecondary>
                                    New File
                                </Button>
                            </NavLink>
                        </div>
                                ...
                                <div>
                                    ...
                                    }
                                    {
                                        sortFiles(files.slice(0, 250)).map((file, index) => (
                                            <FileObjectRow key={file.key} file={file} index={index} length={files.length}/>
                                        ))
                                    }
                                    <MassActionsBar/>
                                </div>
