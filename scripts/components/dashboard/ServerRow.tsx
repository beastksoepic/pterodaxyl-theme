...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    const diskLimit = server.limits.disk !== 0 ? megabytesToHuman(server.limits.disk) : 'Unmetered';
    const memoryLimit = server.limits.memory !== 0 ? megabytesToHuman(server.limits.memory) : 'Unmetered';
    const cpuLimit = server.limits.cpu !== 0 ? server.limits.cpu + ' %' : 'Unmetered';

    return (
        <StatusIndicatorBox as={Link} to={`/server/${server.id}`} className={className} $status={stats?.status} $hoverable>
                ...
                {(!stats || isSuspended) ?
                    ...
                    <React.Fragment>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            ...
                            >
                                { cpuLimit !== 'Unmetered' ? `of ${cpuLimit}` : '(Unmetered)'}
                            </p>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                            ...
                            >
                                { memoryLimit !== 'Unmetered' ? `of ${memoryLimit}` : '(Unmetered)'}
                            </p>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                            ...
                            >
                                { diskLimit !== 'Unmetered' ? `of ${diskLimit}` : '(Unmetered)'}
                            </p>
                        </div>
                    </React.Fragment>
                }
